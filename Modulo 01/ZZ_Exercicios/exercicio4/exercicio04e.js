...
for(var i=0; i<clientes.length; i++){
    for(var i1=0; i1<clientes[i].pedidos.length; i1++){
        clientes[i].pedidos[i1] = { ...clientes[i].pedidos[i1], valor_total: clientes[i].pedidos[i1].quantidade * clientes[i].pedidos[i1].valor_unitario };
    }
}
...
function resumo(clientes){
    ...
    for(var i=0; i<clientes.length; i++){
        total = 0;
        for(var i1=0; i1<clientes[i].pedidos.length; i1++){
            total+= clientes[i].pedidos[i1].valor_total;
        }
        resumo.push({
